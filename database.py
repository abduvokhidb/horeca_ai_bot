...
try:
    # bot username ni deep-link uchun ishlatamiz
...
class Database:
    ...
    def is_user_active(self, telegram_id: int) -> bool:
        with self._conn() as con:
            ...
            r = cur.fetchone()
            return bool(r["a"]) if r else True  # default True — bloklamaslik uchun
    ...
    def remove_employee_by_username(self, username: str) -> bool:
        with self._conn() as con:
            cur = con.cursor()
            cur.execute(
                "SELECT telegram_id FROM users WHERE lower(username)=lower(?) AND role='EMPLOYEE'",
                (username,),
            )
            row = cur.fetchone()
            if not row:
    ...
    # --------- Assignee resolution (username yoki ism) ----------
    def resolve_assignee(self, name_or_username: str) -> Optional[Dict[str, Any]]:
        """@username bo‘lsa — to‘g‘ri, aks holda full_name bo‘yicha eng yaqin moslikni qidiradi."""
        ...
        with self._conn() as con:
            ...
            if key.startswith("@"):
                ...
                return cur.fetchone()
            # Full name exact, so‘ng LIKE (soddalashtirilgan)
            cur.execute("SELECT * FROM users WHERE lower(full_name)=lower(?)", (key,))
            row = cur.fetchone()
            if row:
                return row
            cur.execute("SELECT * FROM users WHERE lower(full_name) LIKE lower(?) ORDER BY length(full_name) ASC LIMIT 1", (f"%{key}%",))
            return cur.fetchone()
    ...
    def set_task_status(self, task_id: int, status: str, by: int, reason: Optional[str] = None) -> bool:
        with self._conn() as con:
            ...
            if status == "accepted":
                cur.execute(
                    "UPDATE tasks SET status='accepted', accepted_at=datetime('now') WHERE id=? AND assigned_to=?",
                    (task_id, by),
                )
            elif status == "rejected":
                cur.execute(
                    "UPDATE tasks SET status='rejected', reject_reason=?, completed_at=NULL WHERE id=? AND assigned_to=?",
                    (reason or "", task_id, by),
                )
            elif status == "done":
                cur.execute(
                    "UPDATE tasks SET status='done', completed_at=datetime('now') WHERE id=? AND assigned_to=?",
                    (task_id, by),
                )
            else:
                return False
    ...
    def mark_task_done_with_report(self, task_id: int, by: int, report: str) -> bool:
        """task_id==0 bo‘lsa — kundalik erkin hisobot sifatida saqlab qo‘yamiz."""
        if int(task_id) == 0:
    ...
    def get_status_overview(self) -> List[Dict[str, Any]]:
        with self._conn() as con:
            cur = con.cursor()
            cur.execute(
                "SELECT * FROM users WHERE role='EMPLOYEE' AND COALESCE(active,1)=1 ORDER BY lower(username)"
            )
    ...
    def create_invite_request(self, user_id: int, username: Optional[str], full_name: Optional[str]) -> int:
        with self._conn() as con:
            cur = con.cursor()
            # agar avval bor bo'lsa yana "pending" yaratmaymiz
            cur.execute(
    ...
    def list_invite_requests(self) -> List[Dict[str, Any]]:
        with self._conn() as con:
            cur = con.cursor()
            cur.execute(
                "SELECT * FROM invite_requests WHERE status='pending' ORDER BY created_at ASC"
            )
            return cur.fetchall() or []
    ...
    def approve_invite_request(self, req_id: int) -> str:
        """Request ni tasdiqlaydi, tokenli invite yaratadi va deep-link qaytaradi."""
        with self._conn() as con:
            cur = con.cursor()
            ...
            cur.execute(
                "UPDATE invite_requests SET status='approved', decided_at=datetime('now') WHERE id=?",
                (req_id,),
            )

            # deep-link
            bot_name = BOT_USERNAME or "<BOT_USERNAME>"
    ...
    def reject_invite_request(self, req_id: int, reason: Optional[str] = None) -> None:
        with self._conn() as con:
            con.execute(
                "UPDATE invite_requests SET status='rejected', reason=?, decided_at=datetime('now') WHERE id=?",
                (reason or "", req_id),
            )

    def create_invite_for(self, username: str, full_name: Optional[str]) -> Tuple[bool, str]:
        """Admin panelidan to‘g‘ridan-to‘g‘ri invite yaratish (requestsiz)."""
        if not username:
            return False, ""
        with self._conn() as con:
            cur = con.cursor()
    ...
    def consume_invite_token(self, token: str, user_id: int, username: Optional[str], full_name: Optional[str]) -> bool:
        """Deep-link orqali kelgan tokenni 'used' qiladi va foydalanuvchini active/EMPLOYEE qiladi."""
        with self._conn() as con:
            cur = con.cursor()
            ...
            cur.execute(
                "UPDATE users SET username=COALESCE(?, username), full_name=COALESCE(?, full_name), role=COALESCE(role,'EMPLOYEE'), active=1 WHERE telegram_id=?",
                (username, full_name, user_id),
            )
            # tokenni ishlatilgan deb belgilash
            cur.execute(
                "UPDATE invites SET status='used', used_by=?, used_at=datetime('now') WHERE id=?",
                (user_id, inv["id"]),
            )
            return True
